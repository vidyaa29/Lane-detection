...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        max=255
        min=1
        self.img_w = 255 * (img_w - min)/(max - min)
        self.n_strips = S - 1
        self.n_offsets = S
        ...
        #lazy conv2d to improve accuracy
        self.lazyconv2d = nn.LazyConv2d(backbone_nb_channels, kernel_size = 3, padding = 1)
        ...
        self.initialize_layer_attention(self.attention_layer)
    ...
    def SqzExc(self, x):
        ...
        y = y.permute(0, 2, 3, 1)
        y = nn.ReLU(nn.Linear(y,y/16))
        y = nn.Sigmoid(nn.Linear(y/16,y))
        y = y.permute(0, 3, 1, 2)
        y = x * y
        return y
    ...
    def forward(self, x, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        #attention_features = self.SqzExc(attention_features)
    ...
    @staticmethod
    def initialize_layer_attention(layer):
        if isinstance(layer, (nn.Conv2d, nn.Linear)):
